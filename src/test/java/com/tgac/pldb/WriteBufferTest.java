...
public class WriteBufferTest {
	...
	public void readsUnionTheBaseAndTheStagedDelta() {
		WriteBuffer lib = WriteBuffer.over(base())
				.asserting(Collections.singletonList(person(null, lval(3L), lval("Kurt")))).get();
	...
	public void anAncestorIsUndisturbedByADescendantsAppend() {
		...
		parent.asserting(Collections.singletonList(person(null, lval(3L), lval("Kurt")))).get();
	...
	public void siblingsForkIndependently() {
		WriteBuffer parent = WriteBuffer.over(base());
		WriteBuffer left = parent.asserting(Collections.singletonList(person(null, lval(3L), lval("Kurt")))).get();
		WriteBuffer right = parent.asserting(Collections.singletonList(person(null, lval(4L), lval("Emmy")))).get();
	...
	public void aStagedDuplicateOfACommittedFactDeliversOnce() {
		WriteBuffer lib = WriteBuffer.over(base())
				.asserting(Collections.singletonList(person(null, lval(1L), lval("Ada")))).get();
